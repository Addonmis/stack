...
export const sign_in = async (ctx) => {
    ...
    const user = await maria.query("select id_user, username, password, id_role from users where username = ?;", [username]).then((rows) => rows);
    ...
    const token = jwt.sign({_id: user[0].id_user}, SECRET);
    ctx.body = {token: token, user_role: user[0].id_role};
...
export const sign_up = async (ctx) => {
    ...
    try{
        ...
        await maria.query("insert into users(username, password, id_role) values(?, ?, ?);", [username, hash, 4]);
